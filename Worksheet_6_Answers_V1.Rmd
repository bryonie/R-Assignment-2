...
marital.status     | married, never married, divorced or separated
...
(1) Before fitting any models, let's see if we can get an intuitive sense of which explanatory variables predict `income`. Here's one possible exploratory plot, with a new `position` argument, `position = "fill"`. What does it do? What can you conclude from it about sex, marital state and income?
...
## answer: `position = "fill"` normalises the heights of bars to proportions,
## instead of counts; this plot shows that being male and being married,
## most importantly, correlate with higher odds of making over $50.000
```
...
## there are 30139 observations in the data set
dim(income)

## there are no NAs in the data set; the various variables are not equally
## distributed, e.g., mostly private sector, more men, mostly US, more < .50k
apply(income, 1, table, useNA = "always")

## older people are more likely to make over $50.000
ggplot(income, aes(x = age, color = income)) +
  geom_line(stat = "count")

## highly educated people are more likely to make over $50.000
ggplot(income, aes(x = education.num, color = income)) +
  geom_line(stat = "count")

## people who work more hours are more likely to make over $50.000
ggplot(income, aes(x = hours.per.week, fill = income)) +
...
First, let's cut our the size of our complete data set a little, to speed things up later. We'll use `dplyr`'s `sample_frac()` function, which lets us automatically sample 20% of our data set - see `?sample_frac()`:
...
We have now designated 70% of `income_frac` as training data, and the remainder, or 30%, as test data.
...
## answer: the data may not be organized randomly; i.e., in this case, records
## might have been added to the file by state; then just taking sequential
## samples might cause the training and test set to be qualitatively different
```
...
set.seed(1)
income_1e4 <- sample_n(income, 10000)
```
...
set.seed(1)
spl_index = createDataPartition(y = tst_income$income, p = 0.60, list = FALSE)
fsplit_1 = tst_income[spl_index, ]
fsplit_2 = tst_income[-spl_index, ]
...
That's pretty great - `train()` is automatically optimising our model's parameters for us; in this case there's just one, *k*, the number of neighboring observations to look at when taking the 'vote' on the prediction, and then reporting back the average cross validated accuracy.
...
set.seed(1)
occupation_knn = train(occupation ~ ., method = "knn", data = trn_income,
  trControl = trainControl(method = 'cv', number = 5))
best_k <- occupation_knn$bestTune$k
...
We need to specify `method = "glm"`, for 'generalised linear model', as well as `family = binomial(link = "logit")`; this is an extra argument to the `"glm"` model fitting procedure. It specifies that this is a two class problem - under or over $50.000 income - and that we wish to solve it with logistic regression.

Logistic regression is a form of regression adapted to classification problems. It assumes a linear relationship between the explanatory variables and *response*, here `income`, and then to fit the model, it converts between the binary response and a continuous probability using the `logit` function.
...
...but that's quite a mouthful, isn't it? It would be much easier to interpret our coeffients if they were indicating the odds of making *more* than $50.000, and if the reference level for `marital.status` was "never.married" instead of "divorced"; as a default, R just goes with what comes first alphabetically.
...
## re-level 'native.country' so that 'united.states' is the reference level
trn_income_f <- trn_income_f %>%
  mutate(native.country = relevel(factor(native.country),
    ref = "united.states"))

## fit the logistic regression model using `train()`
country_lgr = train(native.country ~ ., method = "glm",
  family = binomial(link = "logit"), data = trn_income_f,
  trControl = trainControl(method = 'cv', number = 5))
...
## answer check: the coefficient for raceasian.pac.island should be 3.698
```
...
## comparing accuracy for the best 'k' found for both models
max(income_knn$results$Accuracy)
max(income_knn_as$results$Accuracy)

## answer: yes, there's a reasonably dramatic loss of predictive power!
```
...
set.seed(1)
income_knn_nc = train(income ~ . -capital.gain -capital.loss, method = "knn",
  data = trn_income, trControl = trainControl(method = 'cv', number = 5))
```
...
set.seed(1)
sex_lgr_ehc = train(sex ~ education.num * hours.per.week * income,
  method = "glm", family = binomial(link = "logit"), data = trn_income_f,
  trControl = trainControl(method = 'cv', number = 5))
```
...
## comparing accuracy for the best 'k' found for both models
max(income_knn$results$Accuracy)
max(income_knn_pP$results$Accuracy)

## answer: no, in this case centering & scaling doesn't help!
```
...
Instead, you might be more interested in identifying as many people as possible who are likely to make over $50.000 - in essence, you care about maximizing your 'true positives' while not being terribly concerned about 'false positives'. In machine learning terms, that's called optimising *recall*.

We can get `train()` to do this for us too! Instead of selecting the *k* that optimises accuracy, let's select the *k* that optimises recall. We have to add a new argument, `metric`, and set it equal to `"Recall"`. In addition, we have to tell `trainControl` to keep slightly more & different information about each fold; we set `classProbs` to `TRUE` and `summaryFunction` to `prSummary` - short for, 'precision and recall summary'. Like so:
...
set.seed(1)
occupation_knn_tL = train(occupation ~ ., method = "knn",
  data = trn_income_f, trControl = trainControl(method = 'cv', number = 5),
  tuneLength = 5)

## answer: tuneLength generates a number of parameter values to try equal
## to the 'length' argument that you specify
```

(15) Fit a `"knn"` model predicting `sex` on the basis of all other variables using 5-fold cross validation; optimise `"Precision"` instead of accuracy.

```{r Exercise 15. Optimise precision instead of accuracy.}
## predict sex, optimise precision instead of accuracy
set.seed(1)
sex_knn_mP = train(sex ~ ., method = "knn", data = trn_income,
  trControl = trainControl(method = 'cv', number = 5, classProbs = TRUE,
...
After we've finished fitting our models - trying all the different algorithms, parameters, preprocessing steps and optimisation procedures that we're interested in - and reached a conclusion about which seems to be best, it's time to test its performance on an unseen test set. This is the fairest measure of how well we should expect the  model to perform on new data.
...
About 84%! That's not bad - certainly no worse than we did on the training set. Our model has generalised well to new, unseen data!
...
The actual 'confusion matrix' of the function name is at the top: It shows us that, of the 1808 people in the test set, 229 + 196 actually made over $50.000, and of those 425 people, 229 were correctly identified as such. Conversely, 93 + 1290 made less than $50.000, and of those, 1290 were correctly identified.
...
set.seed(1)
pred_income_trn <- predict(income_knn, trn_income)
trn_confM <- confusionMatrix(pred_income_trn, factor(trn_income$income))
trn_confM

## answer: `income_knn` contains the cross validated accuracy, which uses
## mini-test sets to evaluate accuracy on semi-unseen data; this is really
## just the model predicting data which was also used to train it
```
...
set.seed(1)
pred_income_lgr <- predict(income_lgr, tst_income)
lgr_confM <- confusionMatrix(pred_income_lgr, factor(tst_income$income))
lgr_confM

## extracting the overall accuracy 
lgr_confM$overall["Accuracy"]

## answer: this accuracy is slightly lower than that of the `"knn"` model
knn_confM$overall["Accuracy"]
```
...
income_fit = train(income ~ . -native.country, method = "knn",
  data = trn_income, trControl = trainControl(method = 'cv', number = 3,
    classProbs = TRUE, summaryFunction = prSummary, returnResamp = "all"),
  metric = "Precision", tuneGrid = data.frame(k = seq(3, 15, by = 2)))

## drawing the requested plot
ggplot(income_fit$resample, aes(x = factor(k), y = Precision)) +
  geom_boxplot() +
  scale_x_discrete("value of k") +
